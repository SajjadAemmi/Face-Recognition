...
class MTCNN():
    def __init__(self):
        ...
        # self.retina_face = RetinaFaceModel(True, True)

    def align(self, img, crop_size=(112, 112), return_trans_inv=True):
        # _, landmarks = self.detect_faces(img)
        _, faces, landmarks = self.retina_face.detect(img)

        if len(landmarks) == 0:
            return (None, None) if return_trans_inv else None

        # facial5points = [[landmarks[0][j], landmarks[0][j + 5]] for j in range(5)]
        facial5points = [(landmarks[0][j], landmarks[0][j + 1]) for j in range(0, 10, 2)]
    ...
    def align_fully(self, img, crop_size=(112, 112), return_trans_inv=True, ori=[0, 1, 3], fast_mode=True):
        ori_size = img.copy()
        h = img.size[1]
        w = img.size[0]
    ...
    def detect_faces(self, image, min_face_size=64.0, thresholds=[0.5, 0.5, 0.5], nms_thresholds=[0.7, 0.7, 0.7]):
        """
        Arguments:
