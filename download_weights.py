...
if not os.path.exists(os.path.join(path, 'detection')):
    os.makedirs(os.path.join(path, 'detection'))
...
if __name__ == '__main__':
    output = os.path.join(os.path.join(path, 'detection'), 'mobilenet0.25_Final.pth')
    gdown.download(mobilenet0_25_Final_url, output, quiet=False)

    output = os.path.join(os.path.join(path, 'detection'), 'Resnet50_final.pth')
